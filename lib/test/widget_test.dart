...
void main() {
  ...
  testWidgets('Login Test', (WidgetTester tester) async {
    ...
    await tester.enterText(find.byKey(const Key('UsernameField')).first, 'a');
    await tester.pump(const Duration(seconds: 1));
    await tester.enterText(
        find.byKey(const Key('PasswordField')).first, '12345');
    await tester.pump(const Duration(seconds: 1));
    ...
    await tester.tap(find.byKey(const Key('BtnFeedback')));
    await tester.pump(const Duration(seconds: 2));
    await tester.pumpAndSettle();

    expect(find.byKey(const Key('TextFieldFeedback')), findsOneWidget);
    await tester.pump(const Duration(seconds: 1));

    await tester.enterText(find.byKey(const Key('TextFieldFeedback')).first,
        'APK yg Menarik, tp masih ada beberapa bug');
    await tester.pump(const Duration(seconds: 1));

    await tester.tap(find.byKey(const Key('ButtonAdd')));
    await tester.pump(const Duration(seconds: 1));

    FeedbackModel input = FeedbackModel(
      idUser: null,
      comment: tester
          .widget<TextFormField>(find.byKey(const Key('TextFieldFeedback')))
          .controller!
          .text,
    );
